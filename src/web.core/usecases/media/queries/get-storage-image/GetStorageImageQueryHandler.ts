...
import { GetStorageImageQuery } from './GetStorageImageQuery';
...
@Service()
export class GetStorageImageQueryHandler implements IQueryHandler<GetStorageImageQuery, string> {
    @Inject('media.repository')
    private readonly _mediaRepository: IMediaRepository;

    async handle(param: GetStorageImageQuery): Promise<string> {
        if (!param.appId)
