...
@Service()
export class GetDocumentQueryHandler implements IQueryHandler<GetDocumentQuery, string> {
    @Inject('media.repository')
    private readonly _mediaRepository: IMediaRepository;

    async handle(param: GetDocumentQuery): Promise<string> {
        if (!param.id)
        ...
        if (!media || media.type !== MediaType.DOCUMENT || media.extension !== param.ext.toLocaleLowerCase())
